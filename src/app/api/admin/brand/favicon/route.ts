...
function assetPath() {
  return path.join(getAppStateDir(), 'branding', 'favicon.png');
}
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({
      ok: true,
      exists,
      url: buildPublicUrl('/brand/favicon.png'),
      updated_at: settings.branding?.favicon_updated_at ?? null,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Failed to load favicon';
...
export async function POST(request: Request) {
  try {
    ...
    settings.branding.favicon_updated_at = new Date().toISOString();
    writeSettings(settings);

    return NextResponse.json({ ok: true, url: buildPublicUrl('/brand/favicon.png'), updated_at: settings.branding.favicon_updated_at });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    const settings = readSettings();
    if (settings.branding) delete settings.branding.favicon_updated_at;
    writeSettings(settings);
    ...
    if (message === 'unauthorized') return NextResponse.json({ error: 'Authentication required' }, { status: 401 });
    if (message === 'forbidden') return NextResponse.json({ error: 'Admin access required' }, { status: 403 });
    return NextResponse.json({ error: 'Failed to delete favicon' }, { status: 500 });
  }
}
